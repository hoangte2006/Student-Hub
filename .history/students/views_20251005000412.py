...
#    students = Student.objects.all() # Lấy tất cả đối tượng Student từ cơ sở dữ liệu
    ...
    return render(request, 'students/student_list.html',{
        ...
        'gender': gender,
        'status': status,
        'academic': academic,        'deleted_student': deleted_student,  # Truyền thông tin học sinh đã xóa vào ngữ cảnh để hiển thị trong template
    })
...
def trash_list(request):
    students =  Student.objects.filter(is_deleted=True) # Lấy tất cả học sinh đã bị xoá (is_deleted=True)
    return render(request, 'students/trash_list.html', {'students': students})
