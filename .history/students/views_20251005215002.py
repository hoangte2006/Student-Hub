...
def student_list(request): 
    keyword = request.GET.get('keyword', '') # Lấy giá trị của tham số 'keyword' từ URL (nếu có)
    ...
    academic = request.GET.get('academic', '    ') # Lấy giá trị của tham số 'academic' từ URL (nếu có)

    min_score = request.GET.get('min_score') # Lấy giá trị của tham số 'min_score' từ URL (nếu có)
    max_score = request.GET.get('max_score') # Lấy giá trị của tham số
#    students = Student.objects.all() # Lấy tất cả đối tượng Student từ cơ sở dữ liệu
    ...
    if keyword:
        students = students.filter(name__icontains=keyword) # __icontains: lọc không phân biệt chữ hoa chữ thường

    if min_score:
        students = students.filter(score__gte=min_score) # __gte: lớn hơn hoặc bằng
        
    if max_score:
        students = students.filter(score__lte=max_score) # __lte: nhỏ hơn hoặc bằng
