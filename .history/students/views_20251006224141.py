...
def dashboard(request):
    # Học lực
    academic_raw = Student.objects.filter(is_deleted=False).values('academic').annotate(count=Count('id')) # Đếm số học sinh theo học lực 
    academic_data = [ 
        {'label': Student.ACADEMIC_LABELS.get(i['academic'], 'Không rõ'), 'count': i['count']}
        for i in academic_raw # Chuyển mã học lực sang nhãn dễ hiểu hơn và đếm số lượng, cụ thể là dùng dict ACADEMIC_LABELS trong models.py
        # Dòng for này tương đương với: 
    ] 

    # Giới tính
    gender_raw = Student.objects.filter(is_deleted=False).values('gender').annotate(count=Count('id'))
    gender_labels = dict(Student.GENDER_CHOICES)
    gender_data = [
        {'label': gender_labels.get(i['gender'], 'Không rõ'), 'count': i['count']}
        for i in gender_raw
    ]
