...
def delete_student(request, student_id):
    ...
    request.session['deleted_student'] = {
        'name': student.name,
        'age': student.age,
        ...
        'score': student.score,
        'email': student.email,
        'birthday': str(student.birthday),  # chuyển về chuỗi
    }
...
def undo_delete(request):
    deleted_student = DeletedStudent.objects.order_by('-deleted_at').first()
    if deleted_student:
        Student.objects.create(
            name=deleted_student.name,
            classroom=deleted_student.classroom,
            gender=deleted_student.gender,
            status=deleted_student.status,
            score=deleted_student.score,
            email=deleted_student.email,
            birthday=deleted_student.birthday
        )
        deleted_student.delete()
        messages.success(request, 'Đã khôi phục học sinh thành công.')
    return redirect('student_list')
