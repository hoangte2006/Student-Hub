from django.shortcuts import render, redirect, get_object_or_404, message
...
def delete_student(request, student_id):
    student = get_object_or_404(Student, id=student_id) # Lấy đối tượng Student theo ID hoặc trả về lỗi 404 nếu không tìm thấy
    if request.method == 'POST': # Xác nhận xóa
        student.delete() # Xóa đối tượng học sinh khỏi cơ sở dữ liệu
        messages.success(request, 'Học sinh đã được xóa thành công.') # Thêm thông báo thành công
        return redirect('student_list') # Chuyển hướng về danh sách học sinh sau khi xóa
    return render(request, 'students/delete_student.html', {'student': student})

def dashboard(request):
    total_students = Student.objects.count() # Đếm tổng số học sinh
