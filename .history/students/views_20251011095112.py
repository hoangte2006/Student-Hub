...
def student_list(request):
    ...
    min_amount_paid = request.GET.get('min_amount_paid', '').strip()
    max_amount_paid = request.GET.get('max_amount_paid', '').strip()

    # 2. Tập hợp tất cả vào filters để truyền lại template
    filters = {
        'keyword': keyword,
        'classroom': classroom,
        'gender': gender,
        ...
        'min_amount_paid': min_amount_paid,
        'max_amount_paid': max_amount_paid,
    }
    ...
    if keyword:
        students = students.filter(Q(name__icontains=keyword) | Q(classroom__icontains=keyword))
    if classroom:
        students = students.filter(classroom__icontains=classroom)
    if gender:
        students = students.filter(gender=gender)
    if academic:
        students = students.filter(academic=academic)
        print("🧪 academic =", academic)
        print("🧪 Tổng học sinh sau lọc =", students.count())
    if study_status:
        students = students.filter(study_status=study_status)
    if address:
    ...
    if application_note:
        students = students.filter(application_note__icontains=application_note)
    if payment_status:
    ...
    try:
        if min_score:
            students = students.filter(score__gte=float(min_score))
        if max_score:
            students = students.filter(score__lte=float(max_score))
    except ValueError:
        pass
    ...
    try:
        if amount_paid:
            students = students.filter(amount_paid=float(amount_paid))
        if min_amount_paid:
    ...
    academic_labels = ['Giỏi', 'Khá', 'Trung bình', 'Yếu']
    academic_counts = [37, 33, 16, 76]  # hoặc lấy từ DB
    # 5. Trả về template
