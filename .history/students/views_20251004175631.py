...
def student_list(request): 
    ...
    deleted_student = request.session.pop('deleted_student', None) # Lấy thông tin học sinh đã xóa từ session (nếu có) và xóa nó khỏi session
    ...
    return render(request, 'students/student_list.html',{
        ...
        'gender': gender,
        'status': status,
        'deleted_student': deleted_student,  # thêm dòng n👈 ày
    })
...
#    if request.method == 'POST': # Nếu biểu mẫu được gửi đi (theo phương thức POST) thì xóa học sinh và chuyển hướng về danh sách học sinh sau khi xóa
        request.session['deleted_student'] = {  # Lưu thông tin học sinh vào session để hiển thị thông báo sau khi xóa
            'id': student.id, # Lưu ID của học sinh để có thể khôi phục nếu cần sau này 
            'name': student.name,
...
def undo_delete(request):
    data = request.session.get('deleted_student')
    if data and str(data['id']) == request.POST.get('student_id'):
        Student.objects.create(**data)
        request.session.pop('deleted_student', None)    
    return redirect('student_list')
