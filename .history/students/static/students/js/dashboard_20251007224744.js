...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const canvas = document.getElementById('classAcademicChart');
  const tableBody = document.querySelector('#classTable tbody');

  if (selector && canvas && tableBody && typeof classAcademicData !== 'undefined') {
    ...
    selector.addEventListener('change', function () {
      ...
      classChart.data.datasets[0].data = getChartData(selectedClass);
      classChart.data.datasets[0].label = `Học lực lớp ${selectedClass}`;
      classChart.update();
