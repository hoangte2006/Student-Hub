...
document.addEventListener('DOMContentLoaded', function () {
  if (typeof classAcademicData !== 'undefined') {
    ...
    const getChartData = (cls) => {
      const key = cls.trim(); // loại bỏ khoảng trắng thừa
...
document.addEventListener('DOMContentLoaded', function () {
  if (typeof classAcademicData !== 'undefined') {
    const selector = document.getElementById('classSelector');
    console.log("✅ Selector value ban đầu:", selector.value);
    console.log("✅ Test gọi getChartData:", getChartData(selector.value));
  }
});
