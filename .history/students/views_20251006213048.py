...
def dashboard(request):
        ...
        academic_status_raw = Student.objects.filter(is_deleted=False).values('academic').annotate(count=Count('id'))

        academic_status = [
            {
                'academic': ACADEMIC_LABELS.get(item['academic'], 'Không rõ'),
                'count': item['count']
            }
            for item in academic_status_raw
        ]
    ...
    # Sử dụng values('academic') để nhóm các học sinh theo trạng thái học tập
    # Sử dụng annotate(count=Count('id')) để đếm số lượng học sinh trong mỗi nhóm
    # Kết quả trả về sẽ có dạng: [{'academic': 'Excellent', 'count': 10}, {'academic': 'Good', 'count': 15}, ...]
    academic_status = Student.objects.values('academic').annotate(count=Count('id'))
