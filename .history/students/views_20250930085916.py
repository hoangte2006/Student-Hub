...
def student_list(request): 
    query = resquest.GET.get('q') # Lấy giá trị của tham số 'q' từ URL
    if query:  # Nếu có tham số 'q', lọc danh sách học sinh theo tên
        student = Student.objects.filter(name__icontains=query) # Lọc các học sinh có tên chứa chuỗi query (không phân biệt chữ hoa chữ thường)
