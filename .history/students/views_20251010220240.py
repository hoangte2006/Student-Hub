...
def add_student(request):
    ...
    print("📦 Các trường trong model Student:")
    for field in Student._meta.fields:
        print(f"🔸 {field.name} ({field.get_internal_type()})")
    # In ra toàn bộ trường mà form đang nhận
    print("🧪 Tổng số trường trong form:", len(form.fields))
    for field_name in form.fields:
        field = form.fields[field_name]
        print(f"🔹 {field_name}: {type(field)} | required={field.required}")
