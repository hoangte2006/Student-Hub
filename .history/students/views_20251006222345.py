...
def dashboard(request):
    ...
    academic_status = [ # Chuyển đổi mã học lực thành nhãn hiển thị 
        ...
        for item in academic_status_raw # Chuyển đổi danh sách thô thành danh sách có nhãn hiển thị 
    ]

    # Tương tự cho giới tính
    gender_status_raw = Student.objects.filter(is_deleted=False).values('gender').annotate(count=Count('id'))


    gender_status = [
        {
            'gender': GENDER_LABELS.get(item['gender'], 'Khác'),
            'count': item['count']
        }
