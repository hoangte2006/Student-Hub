...
def student_list(request):  
    ...
    if amount_paid:
        try:
            if application_date: # Kiểm tra nếu người dùng đã nhập ngày
                date_obj = datetime.strptime(application_date, '%Y-%m-%d').date() # Chuyển chuỗi sang đối tượng date
                students = students.filter(application_date=date_obj) # Lọc theo ngày
        except ValueError: # Nếu định dạng ngày không đúng, bỏ qua lỗi
            pass 
