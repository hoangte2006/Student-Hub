...
#    students = Student.objects.all() # Lấy tất cả đối tượng Student từ cơ sở dữ liệu
    ...
    if filters['keyword']: # tái sử dụng bộ lọc nếu có giá trị
        students = students.filter(name__icontains=filters['keyword'])  # cú pháp __icontains để tìm kiếm không phân biệt chữ hoa thường
    classroom = Student.objects.values_list('classroom', flat=True).distinct() # Lấy
